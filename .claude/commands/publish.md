...
Run the TheGates release **step by step**. After EACH step: verify the actual
artifact and print one line `[OK] <step> — <what you confirmed>`. **STOP only at
the gates** below (and immediately on ANY failed verification — never push past a
red check). This is the pipeline that shipped 1.0.2; reproduce it faithfully.
...
## Gate legend
- **[GATE]** — verified, routine: show the user what you confirmed and ask to proceed.
- **[HARD GATE]** — irreversible publish to all users: require an explicit yes, restate the blast radius.
...
4. Print the plan: version, changelog, `RENDERER_RELEASE`, platforms (Linux+Windows launcher via the pipeline; macOS unchanged). **[GATE] Proceed with this release?**
...
- **[GATE] Push to `main`?** → `git push origin main`.
...
- **FIRST-RUN GATE:** this upload path has never been driven by the agent. Show the exact `scp` commands + `unzip -l` of each zip and get an explicit OK before uploading to `thegates:…/renderers/`.
...
- **[HARD GATE]** This publishes to all users (Linux + Windows auto-update; macOS unchanged). The pipeline also packages a Windows zip reusing the existing `TheGates.exe` + a fresh pck. Confirm scope.
...
- **[HARD GATE]** Merge publishes to all Flatpak users. Confirm, then `gh pr merge <#> --repo flathub/io.itch.nordup.TheGates --merge` (keep the release branch, matching prior releases).
...
## Non-negotiables
- Verify+report every step; STOP only at gates and on failed verification.
