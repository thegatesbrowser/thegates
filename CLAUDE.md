...
- Full rules: [`docs/C++ Style Guide.md`](./docs/C++%20Style%20Guide.md).
...
- **Renderer is hardcoded to Vulkan.** `--rendering-driver d3d12` is silently ignored in `TG_RENDERER` builds. Don't propose D3D12 fixes for the renderer without first patching `main/main.cpp`'s hardcoded line. See [`docs/Custom Godot Fork.md`](./docs/Custom%20Godot%20Fork.md).
- **The launcher allocates the shared texture, the renderer imports it.** Counterintuitive direction; do not "fix" it without reading [`docs/External Texture Sharing.md`](./docs/External%20Texture%20Sharing.md) § "Why the launcher allocates."
