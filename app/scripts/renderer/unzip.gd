...
# Handles nested directories — used for the shader cache zip.
static func extract_to_dir(zip_path: String, target_dir: String) -> bool:
	var reader := ZIPReader.new()
	var err := reader.open(zip_path)
	if err != OK: Debug.logclr("Cannot open zip %s to unzip" % [zip_path], Color.RED); return false
	...
	for filename in reader.get_files():
		...
		var file_path := target_dir.path_join(filename)
