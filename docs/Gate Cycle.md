...
- **`gate_entered` ≠ rendering.** `gate_entered` fires when `spawn_target` returns the renderer's pid — process up, sockets bound. But the renderer still has to import the external texture, run `lower_token`, and draw 2-3 frames before its `first_frame` IPC command reaches the launcher and `gate_events.first_frame` fires. On the tutorial gate, debug build, warm machine, this gap is ~3.2s. **Reopening (`open_gate_emit`) inside that gap tears down the old renderer before it ever draws** — the user sees "gate didn't load." Anything that schedules a re-open from `gate_entered` (autotest, programmatic flows, debug shortcuts) should either wait for `first_frame` or budget at least one full first-frame interval. The autotest harness fails the run with exit `33 gate_no_first_frame` if any cycle's `first_frame` is missing — that's the regression signal.
